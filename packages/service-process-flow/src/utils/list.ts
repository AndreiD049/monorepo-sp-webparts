...
export class ListBuilder {
    ...
    public async addTextMultilineField(options: IFieldOptions) {
        const required = Boolean(options.required);
        const field = await this.getList().fields.createFieldAsXml(
            `<Field Indexed='FALSE' Required='${String(required).toUpperCase()}' AppendOnly='FALSE' Description='${options.description}' DisplayName='${options.name}' Format='Dropdown' IsModern='TRUE' IsolateStyles='FALSE' Name='${options.name}' RichText='FALSE' RichTextMode='Compatible' Title='${options.name}' Type='Note'></Field>`
        );
        this.fields.push(field);
        this.onNotify(`Field ${field.data.InternalName} created!`);
    }
    ...
    public async addLookupField(options: ILookupFieldOptions) {
        ...
        const field = await this.getList().fields.createFieldAsXml(
            `<Field Indexed='${String(indexed).toUpperCase()}' Required='${String(required).toUpperCase()}' DisplayName='${options.name}' Format='Dropdown' IsModern='TRUE' IsRelationship='${String(isRelationship).toUpperCase()}' List='${options.ListId}' Mult='${options.type === 'LookupMulti' ? 'TRUE' : 'FALSE'}' Name='${options.name}' ShowField='${options.LookupColumn}' Title='${options.name}' Type='${options.type}'></Field>`
        );
        this.fields.push(field);
        this.onNotify(`Field ${field.data.InternalName} created!`);
    }
