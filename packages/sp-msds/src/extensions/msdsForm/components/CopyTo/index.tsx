...
export const CopyTo: React.FC<ICopyToProps> = (props) => {
    ...
    const [data, setData] = React.useState<{
        Site: string;
        Database: string;
        CustomerNameId?: number | string | null;
    }>({
    ...
    const validate = async (): Promise<boolean> => {
        ...
        if (duplicate.length > 0) {
            setErrors((prev) => ({
                ...prev,
                general: `Product '${props.item.ProductName}' already exists for this customer/site/database.`,
            }));
            isValid = false;
        } else {
    ...
    const handleOk = async (): Promise<void> => {
        try {
            document
                .getElementById('copy-to-ok-button')
                ?.setAttribute('disabled', 'true');
            document
                .getElementById('copy-to-cancel-button')
                ?.setAttribute('disabled', 'true');
            ...
            // Add comment
            const customer = await LookupService.getCustomer(
                props.item.CustomerNameId
            );
            ...
            if (data.Site === props.item.Site) {
                await ItemService.setApprovalStatus(
                    result.data.Id,
                    'Pending',
                    false
                );
    ...
    return (
        <div className={styles.container}>
            <form onSubmit={(ev) => ev.preventDefault()}>
                ...
                <TagPicker
                    ...
                    onChange={async (selected) => {
                        const value =
                            selected && selected.length > 0 ? selected[0].name : null;
            ...
            <div className={styles.footer}>
                ...
                <DefaultButton
                    id="copy-to-cancel-button"
                    onClick={() => hideDialog(DIALOG_ID)}
                >
                    Cancel
                </DefaultButton>
            </div>
