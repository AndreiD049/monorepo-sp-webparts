...
export const Home: React.FC<IHomeProps> = () => {
    ...
    return (
        <div className={styles.container}>
            ...
            >
                <div className={`${styles.border} ${styles.grow}`} style={{ minWidth: 500, overflow: 'auto' }}>
                    <ul className={styles.feedbackList}>
                        {feedbacks.map((feedback, idx) => (
                            <li key={feedback.ID}>
                                ...
                                <span data-type="title" className={styles.grow}>{feedback.Title}</span>
                                <span data-type="created-date">Created: {new Date(feedback.Created).toLocaleDateString()}</span>
                                <span data-type="goto"><IconButton iconProps={{ iconName: 'OpenInNewWindow' }} /></span>
                            </li>
                        ))}
                    </ul>
                </div>
                <div className={`${styles.border}`} style={{ borderLeft: 'none' }}>
                    <img
